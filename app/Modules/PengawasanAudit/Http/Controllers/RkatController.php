...
class RkatController extends \App\Http\Controllers\Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        return view('PengawasanAudit::rkat.default', compact('data', 'company_name'));
    }
    ...
    */
    public function create()
    {
        ...
        return view('PengawasanAudit::rkat.form', compact('rkat_audit', 'company_name'));
    }
    ...
    */
    ...
    {
        // dd($request->tgl_mulai_umum);exit;
        // if($request['tgl_mulai_umum'] === null){
        //     echo 'true';exit;
        // }else{
        //     echo 'false';exit;
        // }
        ...
        $company_name = to_dropdown(Comprof::where('status', 1)->where('id', $request->ljk)->get(), 'id', 'company_name');
        foreach($company_name as $key => $value){
            $company_name = $value;
        }

        $message = ['key' => 'Rkat Audit ', 'value' => $company_name];
        $status = 'error';
        ...
        if ($rkat->save()) {
            ...
            for ($i=0; $i < count($request->input('sequence_umum')); ++$i){
                ...
                $umum->keterangan_umum = $request['keterangan_umum'][$i];
                $umum->save();
            }
            ...
            for ($i=0; $i < count($request->input('sequence_khusus')); ++$i){
                ...
                $khusus->keterangan_khusus = $request['keterangan_khusus'][$i];
                $khusus->save();
            }
        } 
    ...
    * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
    */
    public function edit(Request $request, RkatAudit $rkat_audit)
    {
        $company_name = to_dropdown(Comprof::where('status', 1)->get(), 'id', 'company_name');
    ...
    */
    public function update(RkatRequest $request, RkatAudit $rkat_audit)
    {
        ...
        $company_name = to_dropdown(Comprof::where('status', 1)->where('id', $rkat_audit->ljk)->get(), 'id', 'company_name');
        foreach($company_name as $key => $value){
            $company_name = $value;
        }
    ...
    public function show(Request $request, RkatAudit $rkat_audit)
    {
        // dd($rkat_audit);exit;
        $data = RkatAudit::findOrFail($rkat_audit->id);
        $company_name = to_dropdown(Comprof::where('status', 1)->get(), 'id', 'company_name');

        return view('PengawasanAudit::rkat.show', compact('data', 'company_name'));
    }
